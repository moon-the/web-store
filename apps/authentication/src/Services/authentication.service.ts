...
export class AuthenticationService {
  ...
  async register(user: UserRegisterDTO) {
    ...
    if (!u) {
      ...
      else throw new ConflictException('email đã tồn tại');
    }
  }

  async forgetPassword(forget: ForgotPasswordDTO, host: string) {
    const user = await this.findByUserNameAndEmail(
      forget.username,
      forget.email,
    );
    if (user) {
  ...
  async getToken(t: string) {
    ...
    if (token) {
      ...
      throw new UnauthorizedException('Mã Token không đúng hoặc đã hết hạn');
  }
  ...
  async findToken(token: string) {
    ...
    if (!t) {
      ...
      if (token_) this.cacheManager.set(token, token_.user.userName);
